  ...
  useEffect(() => {
    ...
    const handlePointerUp = (e: PointerEvent) => {
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 30) {
            ...
            // Tap/Click Detection to snap to a lane
            const screenX = e.clientX;
            const screenWidth = window.innerWidth;
            const segmentWidth = screenWidth / laneCount;
            const laneIndex = Math.floor(screenX / segmentWidth);
            const targetLane = laneIndex - maxLane;
            const finalLane = Math.max(Math.min(targetLane, maxLane), -maxLane);
            
            setLane(currentLane => {
                if (currentLane === finalLane) {
                    activateImmortality(); // Tap on current lane triggers immortality
                    return currentLane;
                }
                return finalLane;
            });
